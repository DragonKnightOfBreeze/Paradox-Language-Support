...
- [ ] P3/QOL 提供代码检查（对于脚本文件或CSV文件），以报告当前文件无法匹配到任何 `CwtFilePathMatchableConfig`（默认级别：INFORMATION） - 这意味着当前文件是特殊的，或者缺失相应的规则
- [ ] P3/QOL 将 `ParadoxInlineScriptEditorNotificationProvider` 改为 `InlineScriptUsageInspection`（默认级别：INFORMATION） - 为了对齐已有的 `UnusedInlineScriptInspection`
