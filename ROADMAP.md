...
- [ ] P3/QOL/MAYBE 提供意向，以在作用域的安全调用格式（`owner ?= ...` 或 `owner?= ...`）与显式调用格式（`exists = owner owner = ...`）之间进行转换
- [ ] P3/QOL/MAYBE 提供代码检查，以在作用域的安全调用格式（`owner ?= ...` 或 `owner?= ...`）与显式调用格式（`exists = owner owner = ...`）之间进行转换
- [ ] P3/QOL/MAYBE 提供意向，以在作用域的链式调用格式（`root.owner = ...`）与分离调用格式（`root = { owner = ... }`）之间进行转换 - split/merge - 不检查调用链是否过长
