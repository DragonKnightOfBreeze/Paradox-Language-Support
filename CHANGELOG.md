...
* 完善在线参考文档
* BUG修复：
  * [ ] 修复valueSetValue索引在索引时会被IDE认为栈溢出的问题
* 功能优化：
  * [ ] 编辑本地化文件时提供输入彩色文本、图标等的快捷键（仅在可用的位置生效）
  * [ ] 基于facet或者合成库+自定义设置配置模组的游戏类型、依赖模组列表等配置
* 新增功能：
...
* 功能优化：
  ...
  * [X] 补充内嵌注释的预览文本
* 新增功能：
  * [ ] 通过在脚本文件的第一行添加特殊注释`@path:{path}`，可以强制指定脚本文件相对于模组根目录的路径
    * 仅当PLS无法得到此脚本文件的文件信息，从而进一步解析其中的定义时才允许这样处理
    * 特殊注释不能包含任何其他非空白文本，合法的特殊注释文本示例：`@type:events/dummy.txt`
    * 某些情况下PLS会直接向PSI元素的userData中注入注释文本，以实现同样的效果
  * [ ] 通过在脚本属性的上一行添加特殊注释`@type:{gameType}/{type}`，可以强制指定定义的类型（不要同时指定子类型）
    * 仅当PLS无法得到此脚本文件的文件信息，从而进一步解析其中的定义时才允许这样处理
