...
* 优化：
  ...
  * [ ] 进行代码补全时，在输入`definition_root_key = `之后，应用补全后可以直接自动插入作为定义声明的从句内联模版
  * [ ] 进行代码补全时，在提供定义声明中的定义元素的补全之外，也可以提示定义声明之外的属性名（如果有必要）
* 新增功能： 
  * [X] 支持CWT规则：`stellaris_name_format[xxx]`
    ...
    * （3.6开始支持）如果用引号括起，则是一个特殊的表达式，例如`"{AofB{<imperial_mil> [This.Capital.GetName]}}"`，引号
    * 仍然需要进一步完善……
