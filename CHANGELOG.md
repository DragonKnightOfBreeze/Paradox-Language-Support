...
* [ ] 尝试支持脚本文件的一些高级插值语法（如，`a[[b]c]d`，`a$b$c`，`value:a|b|@c|`）
* [ ] 检查定义的相关本地化和图片是否缺失时，也检查由定义生成的修正的相关本地化和图片是否缺失
* [ ] #41 - 生成本地化时可以另外配置生成哪些本地化
* [ ] #43 - 重命名时重命名相关的本地化和图片的名字（可以配置并且默认启用）

## 1.1.3

* [X] 兼容带有参数的相关本地化和相关图片（不进行进一步的解析，进行代码检查时也不报错）
* [X] 支持渲染嵌入到本地化文本中的任意大小的图片
* [X] 修复构建科技树图时报错的问题
* [X] 修复无法查询自身为文件的定义的问题
* [X] 进行代码补全时，如果需要，自动补上双引号
* [ ] 修复可能无法解析顶层的作用域切换中的表达式对应的规则的问题（`some_scripted_effect = { root = { ... } }`）
* [ ] 修复重命名引用时，即使配置在注释和字符串中查找，也可能无法同步更改CWT文件、脚本文件和本地化文件中的相关文本的问题
* [X] 尝试使用语言注入功能推断脚本参数的传入值对应的CWT规则上下文，从而提供高级语言功能 *
* [X] 尝试使用语言注入功能推断脚本参数的默认值对应的CWT规则上下文，从而提供高级语言功能 *
* [X] 其他优化和BUG修复
