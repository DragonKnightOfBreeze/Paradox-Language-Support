...
* 新增功能
  ...
  * 代码检查（`Code > Inspect Code...`）
    * [ ] 检查作用域上下文与当前作用域是否匹配（警告级别，对于支持且给定了支持的作用域的`trigger` `effect`等，以及`localisationCommand`）
    * [ ] 检查作用域上下文切换是否正确（警告级别，对于各种带作用域的表达式，如`root.owner`，以及`localisationCommandScope`）
