...
* [ ] 复杂表达式中的参数被高亮成了白色（为啥？）
* [ ] 默认游戏类型或者模组游戏类型变更时，检查是否会强制重新解析和索引相关文件（如果有必要）
* [ ] 默认游戏类型或者模组游戏类型变更时，检查是否也需要强制刷新inlayHints（如果有必要）
* [ ] 偏好语言区域变更时，检查是否会强制刷新inlayHints
* [ ] 模组或游戏配置未提交更改时，检测是否实际未更改
* [ ] 使用合成库后，快速文档等地方显示的库名是空的
...
* 功能优化
  ...
  * [ ] 可以从启动器模组列表文件一键导入模组依赖列表
  * [ ] 可以导出模组依赖列表为启动器模组列表文件
