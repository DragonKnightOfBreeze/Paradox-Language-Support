...
* 新增功能：
  ...
  * [X] 添加检查：值集中的值（`some_flag`）被设置但未被使用（例如，有`set_flag = xxx`但没有`has_flag = xxx`。）
  * [X] 添加检查：值集中的值（`some_flag`）被使用但未被设置（例如，有`has_flag = xxx`但没有`set_flag = xxx`。）
  * [X] 添加内嵌提示：本地化图标（渲染出选用的内嵌图标，如果对应图标的大小合适）
...
* ［长期］完善CWT配置支持：
  ...
  * [ ] 支持高亮`definitionName`,`complexEnumValueName`（对应的PSI元素可能本身就对应着特定的CWT规则，需要同时高亮出来） *
  * [ ] 优化：更好地兼容嵌套的定义
