...
* 新增功能： 
  * 代码补全（`Code > Code Completion`）
    * [X] 当可能正在输入一个定义名（非引用，作为顶级属性名）时，可以自动插入后面的等号、花括号以及从句内联模版，并将光标放到合适的位置（需要手动调用代码补全）
    * [X] 当可能正在输入一个本地化名（非引用）时，可以自动插入后面的冒号、数字以及双引号，并将光标放到合适的位置（需要手动调用代码补全）
    * [ ] 当可能正在输入一个封装变量名（非引用）时，提供基于已索引的封装变量名的代码补全，以便进行重载
    * [ ] 当可能正在输入一个定义名（非引用）时，提供基于已索引的定义名的代码补全，以便进行重载
    * [ ] 当可能正在输入一个本地化名（非引用）时，提供基于已索引的本地化名的代码补全，以便进行重载
