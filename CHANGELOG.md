...
* 新增功能：
  * [X] 完善支持处理生成的修饰符（`modifier`），以及相关的引用解析、代码补全、代码高亮等功能
    * [ ] 支持通过Stellaris的`economic_category`生成修饰符（原版游戏会生成的照常支持）
    * [ ] 提供修饰符解析器的扩展点，便于后续扩展
  * [ ] 操作（`Action`）
    * [X] 在文件上使用`Goto Implementations`操作可以导航到同名的重载或者被重载的文件
  ...
  * 代码检查（`Code > Inspect Code...`）
    * [ ] 缺少的传参（在调用表达式和SV表达式中，如果参数不存在默认值且未传递，则认为缺少传参）
    * [ ] 推断的`inline_script`的位置存在冲突（对应的CWT规则的路径不一样，此时显示警告，不使用推断的信息进行引用解析等）
  * 装订线图标（`Gutter Icon`）
    * [ ] 也为封装变量（`@a = 1`，本地化&全局）提供装订线图标，以便导航到同名的重载或者被重载的封装变量
