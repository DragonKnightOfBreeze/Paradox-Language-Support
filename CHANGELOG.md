...
* [X] 优化：可以配置处理图片时使用的工具（默认且内置：Texconv，可选：Image Magick、Paint.NET） / OP: Can configure the tool used to process images (Default and built-in: Texconv, Available: Image Magick or Paint.NET)
...
* [X] 新功能：如果可用，提供操作以直接使用Image Magick或者Paint.NET打开图片 / NEW: If available, provide actions to directly to open images by Image Magick or Paint.NET
...
* [ ] 新功能：魔提供对魔法注释的初步支持，用于批量处理和生成脚本文件和本地化文件的内容
