...
* [ ] 在本地化文件中允许`KEY:"..."`这样没有冒号和引号之间没有空格的写法，以后可能会提供代码检查
* [ ] 实现扩展点以在某些极个别情况下基于另外的逻辑获取脚本表达式对应的CWT规则以及作用域上下文（如，对于`complex_trigger_trigger`）
...
ethic = ethic_$ETHIC$ # 脚本参数作为某个脚本表达式的一部分，这个脚本表达式中仅存在这唯一一个脚本参数，且可以得到这个脚本表达式对应的CWT规则 （TODO）
```
