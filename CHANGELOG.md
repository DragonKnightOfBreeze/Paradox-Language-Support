...
* [ ] 尝试推断脚本参数对应的CWT规则，并且如果推断结果是唯一且确定的，对于传入的参数值应用高级语言功能以及代码检查
...
* [X] 更新CWT规则文件以适配Stellaris 3.8（未作充分验证）
* [ ] 脚本参数可以直接在用双引号括起的字符串中使用（但是如何转义？）
* [ ] 脚本参数也可以在路径引用中使用，包括内联脚本的路径引用
* [ ] 内联脚本可以在脚本文件中任意位置使用，包括定义声明之外
* [ ] 实现扩展点以为特定名字的定义指定不同的CWT规则（如3.8开始某些特定的`game_rule`）
