...
* [ ] 添加代码检查：未正确进行重载的全局封装变量（例如，游戏文件`test.txt`中声明了一个全局封装变量`@v`，而模组在文件`zzz_test.txt`中进行了重载） 
* [ ] 添加代码检查：未正确进行重载的定义 （例如，游戏文件`test.txt`中声明了一个事件`e.1`，而模组在文件`zzz_test.txt`中进行了重载）
* [ ] 兼容直接在脚本文件中使用内联脚本，而非必须在定义声明中（或者内联后在定义声明中）
* [ ] 兼容直接在键或字符串中使用参数条件块（`foo_[[bar]bar]_desc`） - 兼容语法以及相关功能
* [ ] 尝试在更多情况下推断脚本参数对应的CWT规则（上下文） - 支持通过语法注入的方式推断多行脚本参数对应的CWT规则上下文
* [ ] 尝试在更多情况下推断脚本参数对应的CWT规则（上下文） - 支持脚本参数作为某个表达式的一部分，此表达式中仅存在这唯一一个脚本参数，且可以推断此表达式对应的CWT规则的情况
* [ ] 修复 #26 P2 - 应当先尝试将`k = {...}`解析为内联脚本调用，再解析为定义声明

## 1.1.0

* [X] 修复 #32 - 重命名内联脚本的文件名时，相关缓存未正确刷新
* [ ] 修复 #33 - 重命名内联脚本的文件名时，相关的表达式未正确更新文本
...
* [X] 修复 #26 P1 - 在脚本文件中，应当允许多行的用双引号括起的字符串
* [X] 修复 #28 - 生成科技树时报错
