...
> * 定义的本地化名字指：定义的最相关的本地化文本（基于`cwt`规则文件，注有`## primary`的，或者从特定名字的定义属性的值推断来的，这特定名字可以在设置页面中设置）
...
* [ ] 完善内嵌提示 *
  * [ ] 定义的内嵌提示（定义的名字和类型、本地化名字）
  * [ ] 变量引用的内嵌提示（变量的值）
  * [ ] 定义引用的内嵌提示（定义引用的名字和类型、本地化名字）
  * [ ] 本地化引用的内嵌提示（截断的本地化引用的渲染后文本）
...
* [ ] 完善对CWT配置文件的支持，完善相关功能
  * [ ] 支持在`config/cwt/${gameType}.pls`目录中提供插件额外提供的规则文件（如果可以仅添加不修改，与之前的配置合并）
