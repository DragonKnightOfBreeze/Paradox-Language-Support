...
* [ ] 优化：优化定义规则与复杂枚举规则的路径匹配的逻辑（基于`path`字段的值，可以是一个ANT路径表达式，可以有多个）［待定］
...
* [ ] 新功能：支持切换快速文档中的本地化使用的语言区域（如果可用，打开快速文档后，在右键菜单或者右下角更多菜单中，点击`Change Localisation Locale`）
* [ ] #56 新功能：支持从数据库对象生成的概念（同时提供相应的代码补全、代码导航与代码检查）
* [ ] #78 优化：支持通过扩展的CWT规则文件为动态值（如`event_target`）指定作用域上下文 - 提供更加完善的支持（相见参考文档）
