...
* 功能优化
  * [ ] 完善对图表的支持 - 支持通过可见性和作用域过滤显示结果，提供另外的更加详细的配置页面（需要专业版IDE）
...
* [ ] 可以基于特殊位置（文件开始、直接在定义之前、直接在定义成员之前）的特殊注释禁用指定ID的代码检查
* [ ] 在游戏目录或模组根目录下创建目录时，可以提示目录名称（基于已有的包含脚本文件、本地化文件或者DDS文件的目录）
* [ ] 支持本地化参数（即本地化文件中的解析为脚本参数而非本地化引用的`$PARAM$`）
