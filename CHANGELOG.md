...
* 新增功能： 
  * 代码补全（`Code > Code Completion`）
    * [ ] 当可能正在输入一个定义的顶层属性名且该顶层属性名即是定义名时，可以自动插入后面的等号、花括号以及从句内联模版
    * [ ] 当可能正在输入一个封装变量名（scriptedVariableName）时，提供基于已索引的封装变量名的代码补全，以便进行重载
    * [ ] 当可能正在输入一个定义名（definitionName）时，提供基于已索引的定义名的代码补全，以便进行重载
    * [ ] 当可能正在输入一个本地化名（definitionName）时，提供基于已索引的本地化名的代码补全，以便进行重载
