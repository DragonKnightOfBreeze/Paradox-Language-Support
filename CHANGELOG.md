...
* [ ] 更好地兼容在脚本文件中的作为键或值的字符串中使用参数（如，`$p1$s = s$p2$s`）
* [X] 更新CK3、EU4、IR和VIC3的CWT规则文件
* [X] 添加代码检查：（对于脚本文件）在不支持的地方使用了参数（仅限在scripted_effect/scripted_trigger）等支持参数的定义声明中以及内联脚本中使用
* [ ] 添加代码检查：（对于脚本文件）在不支持的地方使用了封装变量（不能在asset文件里面使用） 
* [ ] 添加代码检查：（对于脚本文件）在不支持的地方使用了内联数学表达式（不能在asset文件里面使用，在每个scripted_trigger/scripted_effect中最多只能使用1次）
