...
* [ ] 优化对于`CwtTypeConfig`与`CwtComplexEnumConfig`的路径匹配
  * [ ] 来自CWTools：`path` `path_file` `path_extension` `path_strict`（其中`path`属性可以重复）
  * [ ] 扩展，1.3.20新增：`path_pattern`（填写ANT表达式，可以重复）
...
* [ ] 需要确认parameter与modifier的缓存能够正确生效，以及基于文件路径的modificationTracker都能正确生效
