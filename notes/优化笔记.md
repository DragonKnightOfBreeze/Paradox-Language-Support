...
* 在`ParadoxScriptExpressionReferenceProvider`与`ParadoxLocalisationExpressionReferenceProvider`中适用缓存（`CachedValue`）
* 如果需要获取的仅是脚本或者本地化的表达式的引用列表，考虑使用扩展属性`PsiElement.expressionReferences`，而非`PsiElement.references`
