...
* 如果dataType为`file_path`，则需要匹配任意路径（相对于游戏或模组根路径），并且对应一个文件。
* 如果dataType为`file_path[some/path/]`，同上，但是需要在加上前缀`some/path/`后进行匹配。
* 如果dataType为`file_path[some/path/,ext]`，同上，但是需要在加上前缀`some/path/`和后缀`.ext`后进行匹配。
...
* 如果dataType为`value_field`，则需要匹配SV表达式。（示例：`value:xxx|xxx|xxx|` `modifier:xxx` `trigger:xxx`）
...
在原始CWT规则文件，以下两个规则中的`enum[scripted_effect_params]`对应scripted_effect/scripted_trigger等类型的定义声明中的参数
