...
**属性** 的结构为 `<key> <sep> <value>`，其中分隔符 `<sep>` 可以是 `=`（等于）、`==`（等于）、`!=`（不等于）或 `<>`（不等于，与 `!=` 等价）。
...
**内联数学表达式** 以 `@[` 开头、以 `]` 结尾，如 `@[ 1 + 2 * var ]`。支持的运算符包括 `+`、`-`、`*`、`/`、`%`，以及一元正负号、绝对值 `|expr|` 和括号 `(expr)`。表达式中的因子可以是数字、封装变量名或参数。
...
**参数** 的语法为 `$<name>$` 或 `$<name>|<argument>$`，其中 `|<argument>` 部分为格式化参数（例如用于指定显示格式），不同于脚本语言中参数的默认值语义。`<name>` 的最常见形式是本地化键名（引用另一条本地化文本），由字母、数字、下划线、点号、连字符和单引号组成。此外，`<name>` 在解析层面也可以是命令表达式或封装变量引用——例如 `$@my_var$` 表示引用封装变量 `my_var` 的值。

**图标** 的语法为 `£<icon>£` 或 `£<icon>|<frame>£`。`<icon>` 为图标名称（对应 GFX 定义），`<frame>` 为可选的帧索引。图标名和帧索引均可参数化（包含 `$...$`）。

**命令** 的语法为 `[<text>]` 或 `[<text>|<argument>]`，常用于调用作用域链和 `Get...` 类方法，例如 `[Root.GetName]`。命令文本可以包含参数。注意 `[[` 被视为转义的方括号（输出字面量 `[`），而非命令。
...
文本中的双引号通常不需要转义。插件使用启发式规则来判断引号是否为闭合引号：如果当前行后方还有另一个 `"` 字符，当前 `"` 被视为文本内容；否则视为闭合引号。
...
The plugin assumes backslash escaping (e.g., \") for quoted columns,
since the game engine's actual CSV escaping behavior is undocumented.
Standard CSV uses "" for escaping, but since Paradox CSV also supports # comments (non-standard),
backslash escaping is assumed here.
