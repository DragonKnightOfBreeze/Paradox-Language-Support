...
插件进一步集成了对 Markdown 的支持，针对模组开发场景，优化了对链接、内联代码和代码块的处理。

### 链接 {#md-link}

通过特定格式的链接文本，插件可以尝试将 Markdown 链接解析为匹配的目标引用（定义、本地化等），
...
> [!warning]
> 本章节的功能以及 Diagrams 插件仅在专业版 IDE 中可用（例如：IntelliJ IDEA Ultimate）。

插件提供了数种图表（Diagrams），可用于展示特定类型的定义的信息，以及相互之间的关系。
...
1. 用于设置要显示的节点元素。从左到右依次是类型、属性、展示名字（事件标题）。
2. 用于按查询作用域过滤要显示的节点。例如，仅显示已打开的文件中的事件对应的节点。
3. 用于打开图表的设置页面。从左到右依次是 Diagrams 插件的设置页面与插件的设置页面。
...
1. 用于设置要显示的节点元素。从左到右依次是类型、属性、展示名字（科技名）、图形展示（以科技卡的形式）。
