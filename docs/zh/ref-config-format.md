...
规则的来源与覆盖机制详见[规则系统](config.md)文档中"规则分组"与"覆盖策略"相关章节。
...
- `base_name`：基本类型名。如果存在则表示继承另一类型的部分语义。
- `name_field`：名称字段键。用于解析定义名。如果为空字符串，则强制匿名；如果为 `-`，则从属性值解析定义名；否则从对应名字（忽略大小写）的子属性值解析定义名。
- `name_from_file`：设为 `yes` 时表示从文件名解析定义名（去除文件扩展名）。
...
- `## display_name`（选项/未使用）：子类型的显示名字（选项注释）。
...
- 别名唯一键由 `name:subName` 组成；重复定义将按覆盖策略 / 优先级处理。
...
- `aliases`：该作用域的别名集合。可以为空。用于作用域的匹配与合并（匹配与合并时兼容别名形式）。
- `is_subscope_of`：该作用域的父作用域。可以为空。用于作用域的匹配与合并（匹配父子作用域时采用并集策略，合并父子作用域时会提升子作用域到父作用域）。
...
宏规则用于描述脚本文件中区别于一般抽象的特殊语言构造，并提供额外的用于提示和验证的元数据。
这些语言构造会改变游戏运行时的脚本解析器的行为，从而改变、扩展或复用已有的脚本片段。
不同的宏可以拥有不同的规则结构。

这里的宏特指**语义宏**，区别于一般的**语法宏**。
这意味着它们必须经过语义解析才能确定，不能直接通过语法分析确定，在语法分析阶段只能被识别为某个（或某些）语句或表达式。

目前涉及的宏包括：
- **内联脚本（inline script）**：（Stellaris）会在解析阶段被替换为目标文件的内容，且可以指定参数。
- **定义注入（definition injection）**：（VIC3 / EU5）会在解析阶段对目标定义的声明进行注入或替换，且可以指定模式以决定具体行为。
...
匹配与常量值完全相同的表达式。
如果表达式未用引号括起，不能用来匹配布尔关键字（`yes` 和 `no`）。
...
- `@` 之后的是规则路径，子路径 `-` 匹配所有直接的值，其余情况会作为通配符（忽略大小写，使用 `any` 或 `*` 匹配任意字符，使用 `?` 匹配单个字符）匹配对应键的所有属性。
