...
在 IDE 的设置页面中，点击 `语言与框架 > Paradox Chronicle > 规则`，可以打开规则的设置页面。
...
插件提供的许多语言功能都是由 CWT 规则文件驱动的，这些规则文件属于不同的规则分组。

规则分组可以有不同的来源，而对于同一来源的规则分组，又区分为各个游戏类型的规则分组，以及所有游戏类型通用的规则分组。

你可以在插件的设置页面（`Paradox Chronicle > Config`）中按需启用或禁用各类规则分组。
...
- 位置：`config/{gameType}`
- 位于插件压缩包中的插件 jar 包中。
- `{gameType}` 为游戏类型 ID[^1]，对于通用的规则分组则为 `core`。
- 通用的内置规则分组总是会被启用。
...
- `{dirName}` 为仓库目录的名字，对于通用的规则分组则为 `core`。
...
- [CWTools 的指南文档](https://github.com/DragonKnightOfBreeze/Paradox-Language-Support/blob/master/references/documents/cwtools/guidance.md)
...
[^1]: 目前有以下可选值：`stellaris`, `ck2`, `ck3`, `eu4`, `eu5`, `hoi4`, `ir`, `vic2`, `vic3`。对于通用的规则分组则为 `core`。