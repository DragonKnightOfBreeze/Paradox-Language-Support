...
双引号字符串中使用反斜杠 `\` 进行转义。常见的转义序列包括 `\"`（双引号）和 `\n`（换行）。未加引号的键和字符串不需要转义。
...
双引号字符串中使用反斜杠 `\` 进行转义。常见的转义序列包括 `\"`（双引号）、`\n`（换行）和 `\\`（反斜杠自身）。未加引号的键和字符串不需要转义。
...
方括号 `[` 是命令的开始标记。如果需要在文本中输出字面量 `[`，应使用 `[[` 进行转义。`[[` 不会被解析为命令，而是输出一个 `[` 字符。
...
Paradox CSV 同样使用 `#` 作为注释标记（标准 CSV 不支持这一语法），考虑到这一点，插件假定 Paradox CSV 与脚本文件一样使用反斜杠 `\` 进行转义（例如 `\"`），而非标准 CSV 中常见的双引号转义（`""`）。
