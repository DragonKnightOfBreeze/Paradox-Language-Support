...
在语义上，当分隔符左侧的属性键（示例中是 `owner`）表示一个作用域时：

- `owner ?= v` 等同于 `exists = owner owner = v`
- `owner? = v` 等同于 `exists = owner owner = v`
...
与字符串表达式（键和字符串）一样，他们也可以包含参数（如 `$PARAM$`）和内联参数条件块（如 `[[PARAM]v]`），详见[进阶语法](#script-advanced)一节。
...
**参数条件块**（parameter condition block）的语法为 `[[<expression>] <members...> ]`，其中 `<expression>` 可以是参数名或 `!<参数名>`（取反）。
它用于根据参数是否存在来条件化地包含一组成员。

**内联参数条件块**（inline parameter condition）用于字符串表达式（键和字符串）或封装变量引用内部的条件片段。
...
以下是一个更完整的示例，进一步展示了分隔符、封装变量、参数和参数条件块等语法的组合使用：
