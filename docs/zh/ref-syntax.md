...
其中：
- **属性**和**块值**（即非属性值的值）被统称为**成员**（member），它们可以多次出现在文件顶层、块或者参数化快中，并且可以重复出现和混合出现。
- **属性的键**和**值**被统称为**表达式**（expression）。它们在不同的位置和上下文中拥有不同的语义。
...
- 未用双引号包围时，不能包含空白以及一些特殊字符（`@` `#` `$` `=` `<` `>` `!` `?` `{` `}` `[` `]` `"`）。
- 可以包含特定的转义字符（如 `\"` `\n`）。
- 另外，也可以包含参数（如 `$PARAM$`）和内联参数化快（如 `[[PARAM]v]`），详见[进阶语法](#script-advanced)一节。
...
与字符串表达式（键和字符串）一样，他们也可以包含参数（如 `$PARAM$`）和内联参数化快（如 `[[PARAM]v]`），详见[进阶语法](#script-advanced)一节。
...
**参数**（parameter）的语法为 `$name$` 或 `$name|default_value$`（带默认值）。
...
例如在 `"prefix[[PARAM]_$PARAM$]_suffix"` 中，`[[PARAM]_$PARAM$]` 部分仅在参数 `PARAM` 存在时才会展开。
...
以下是一个更完整的示例，进一步展示了分隔符、封装变量、参数和参数化快等语法的组合使用：
