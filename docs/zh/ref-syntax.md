...
<!--
@doc-meta
本文档是 Paradox Chronicle 插件所支持的四种语言的语法参考。
...
文档内容基于 Paradox Chronicle 插件的解析实现编写，在绝大多数情况下与游戏引擎的实际行为一致，但由于游戏引擎的解析细节并非完全公开，个别边界情况可能存在差异。
...
其中，CWT 与 Paradox Script 共享相似的基础语法结构（属性、值、块），但在分隔符种类、注释系统和高级语法上有所不同，本文档将分别说明。
...
CWT 文件由三种基本元素组成：**属性**（property）、**值**（value）和**块**（block）。
空白与换行用于分隔标记，空行可以自由出现。
支持以 `#` 开头的行注释，以及数种基于 `#` 的注释变体。

其中：
- **属性**和**块值**（即非属性值的值）被统称为**成员**（member），它们可以多次出现在文件顶层或者块中，并且可以重复出现和混合出现。
- **属性的键**和**值**被统称为**表达式**（expression）。它们在不同的位置和上下文中拥有不同的语义。

**属性** 的结构为 `<key> <separator> <value>`，其中分隔符 `<separator>` 可以是以下之一：

- `=` - 逻辑等于/赋值。
- `!=` `<>` -  逻辑不等于。
- `==` - 类似 `=`，但表示匹配的脚本属性应使用比较运算符（如 `=` `!=` `>=`），而不是赋值运算符（如 `=` `?=`）。

如果规则文件中使用了 `==`，插件默认不会严格检查匹配的脚本属性的分隔符是否正确。
这项检查可在设置页面中显式启用，然而，取决于规则的完善程度，这可能引发额外的误报。
...
**整数** 如 `10`、`-5`、`042`。允许前导的正负号和前导的零。

**浮点数** 如 `1.0`、`-0.5`、`.25`。允许省略整数部分或小数部分。

**字符串** 如 `var` `"hello world"` `"line\nnew line"`。其中可以包含转义字符。

**块** 使用花括号 `{ ... }` 包围，内部可以混合包含成员（属性和值）和注释。可以嵌套多层。
...
**选项注释** 以 `##` 开头，用于为紧随其后的成员（属性、值或块）附加元信息。
选项注释的内容语法与属性一致，采用 `<optionKey> <separator> <optionValue>` 的形式。
常见的选项注释包括 `## cardinality = 0..1`（声明基数约束）、`## severity = warning`（声明检查严重级别）、`## push_scope = country`（声明推入的作用域）等。

**文档注释** 以 `###` 开头，用于为成员提供说明文本，
这些文本会在代码补全和快速文档中展示。
特别地，对于以 `####`（或更多 `#`）开头的文档注释，插件会将注释文本视为 Markdown 文本。

### 转义 {#cwt-escaping}

使用 `\` 转义字符串表达式（键和字符串）中的特殊字符。
常见的转义序列包括 `\"`（双引号）、`\n`（换行）和 `\\`（反斜杠自身）。

### 综合示例 {#cwt-example}

以下示例来自插件的测试数据文件，展示了 CWT 的典型用法：
...
脚本语言的基础语法与 CWT 相似，同样由属性、值和块构成，但在分隔符种类、值类型和高级语法上有较多差异，下文将逐一说明。

### 基本元素 {#script-basics}

脚本文件主要由三种基本元素组成：**属性**（property）、**值**（value）和**块**（block）。
空白与换行用于分隔标记，空行可以自由出现。
支持以 `#` 开头的行注释。

其中：
- **属性**和**块值**（即非属性值的值）被统称为**成员**（member），它们可以多次出现在文件顶层、块或者参数化块中，并且可以重复出现和混合出现。
- **属性的键**和**值**被统称为**表达式**（expression）。它们在不同的位置和上下文中拥有不同的语义。
...
- `!=` `<>` -  逻辑不等于。
- `<` `>` `<=` `>=` - 各种比较运算符。
- `?=` - 安全赋值，赋值运算符的一种安全变体（仅 CK3/VIC3/EU5 支持）。
- `? =` - 安全调用赋值，赋值运算符的一种安全变体（仅 Stellaris v4.4+ 支持，与左侧的属性键之间不允许有任何空白）。

在语义上，当分隔符左侧的属性键表示一个作用域时：

- `from.owner ?= v` 会在执行作用域调用之前，检查 `from` 和 `from.owner` 对应的作用域是否存在（`exists = x`）。
- `from.owner? = v` 会在执行作用域调用之前，检查 `from` 和 `from.owner` 对应的作用域是否存在（`exists = x`）。
...
- 未用双引号包围时，不能包含空白以及一些特殊字符（`@` `#` `$` `=` `<` `>` `!` `?` `{` `}` `[` `]` `"`）。
- 可以包含特定的转义字符（如 `\"` `\n`）。
- 另外，也可以包含数种高级插值语法（如参数 `$PARAM$` 和内联条件化块 `[[PARAM]text]`），详见[高级语法](#script-advanced)一节。
...
**字符串** 如 `var` `"hello world"` `"line\nnew line"`。其中可以包含转义字符以及数种高级插值语法。

**颜色** 是一种整体值，格式为颜色类型关键字后跟花括号包围的空白分隔数字，如 `rgb { 255 0 0 }`、`hsv { 0.0 1.0 1.0 }`、`hsv360 { 180 80 100 }`。

**块** 使用花括号 `{ ... }` 包围，内部可以混合包含成员（属性和值）、封装变量、参数化块和注释。可以嵌套多层。

**封装变量引用** 以 `@` 开头后跟变量名，如 `@my_var`，用于引用先前声明的封装变量。
封装变量引用可以出现在多种上下文中：作为独立的值（`cost = @my_var`）、嵌入在未加引号的字符串中（`key = prefix_@my_var_suffix`）、以及在内联数学表达式内作为因子（`@[ base_cost * bonus ]`，此时不带 `@` 前缀）。
与字符串表达式（键和字符串）一样，它们也可以包含数种高级插值语法（如参数 `$PARAM$` 和内联条件化块 `[[PARAM]text]`），详见[高级语法](#script-advanced)一节。
另外，封装变量引用同样可以在本地化文本的参数中使用（如 `$@my_var$`）。

**内联数学表达式** 以 `@[` 或 `@\[` 开头、以 `]` 结尾，如 `@[ 1 + 2 * var ]`。
支持的运算符包括 `+`、`-`、`*`、`/`、`%`，以及一元正负号、绝对值 `|expr|` 和括号 `(expr)`。
表达式中的因子可以是数字、封装变量引用（不带 `@` 前缀）或参数。

### 封装变量 {#script-scripted-variables}

封装变量（scripted variable）通过 `@<name> = <value>` 的形式声明，其中值可以是布尔、整数、浮点数、字符串或内联数学表达式。声明后可通过 `@<name>` 在其他地方引用。

封装变量名以 `@` 开头，名称部分由字母、数字和下划线组成。
与字符串表达式（键和字符串）一样，它们也可以包含数种高级插值语法（如参数 `$PARAM$` 和内联条件化块 `[[PARAM]text]`），详见[高级语法](#script-advanced)一节。

### 高级语法 {#script-advanced}

以下语法通常仅在特定定义（如封装效果、封装触发器、脚本值）的声明上下文中生效或被游戏引擎评估。

**参数**（parameter）的语法形式为 `$PARAM$` 或 `$PARAM|default_value$`（带默认值）。
参数可以视为字面量或标识符的一种通配形式，且可以额外指定默认值。
参数名必须以字母或下划线开头，后续可包含字母、数字和下划线。
作为一种高级插值语法，它们可以出现在字符串表达式（属性键和值）、封装变量名、封装变量引用等位置，且可以任意组合使用。

**内联参数化块**（inline conditional block）的语法形式为 `[[PARAM]text]`。
内联参数化块可以视为字符串或标识符的一种条件通配形式，仅当上下文中存在对应的参数时（可以取反），其中的文本才会展开。
例如，`[[PARAM]text]` 仅在参数 `PARAM` 存在时才会展开为 `text`，对于 `[[!PARAM]text]` 则反之。
作为一种高级插值语法，它们可以出现在字符串表达式（属性键和值）、封装变量名、封装变量引用等位置，且可以任意组合使用。

**参数化块**（conditional block）类似内联条件化块，但发生在块级别。
仅当上下文中存在对应的参数时（可以取反），其中的内容（例如一组属性）才会展开。
例如，`[[PARAM] key = value ]` 仅在参数 `PARAM` 存在时才会展开为 `key = value`，对于 `[[!PARAM] key = value ]` 则反之。

### 转义 {#script-escaping}

使用 `\` 转义字符串表达式（键和字符串）中的特殊字符。常见的转义序列包括 `\"`（双引号）、`\n`（换行）和 `\\`（反斜杠自身）。

### 综合示例 {#script-example}

以下示例来自插件的测试数据文件，展示了 Paradox Script 的典型用法：
...
以下是一个更完整的示例，进一步展示了分隔符、封装变量、参数和参数化块等语法的组合使用：
...
本地化文本中的双引号通常不需要转义。
插件使用启发式规则来判断引号是否为闭合引号：如果当前行后方还有另一个 `"` 字符，当前 `"` 被视为文本内容，否则视为闭合引号。

方括号 `[` 是命令的开始标记。如果需要在文本中输出字面量 `[`，应使用 `[[` 进行转义。

### 综合示例 {#loc-example}

以下示例来自插件的测试数据文件，展示了本地化文件的各种富文本标记：
...
l_english:
 ...
 text_with_variants:0 "Text|||A|||t1:B|||t2,t3:C"
 text_with_tags:0 "Text&!t1,t2"
 text_with_context_tags:0 "$1&t$ [From.GetName&t]"
```
...
列值可以不加引号或用双引号包围。允许空列（即两个相邻的分号之间没有内容）。
