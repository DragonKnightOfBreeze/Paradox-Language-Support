...
以清晰、准确、可操作为原则，保持与实际用法及工具链表现的一致，
并将随着生态与版本演进持续打磨与完善。
...
> [!tip]
> 如果文档注释以 `####` （或者更多的 `#`）开始，插件会直接将注释文本渲染为 HTML。
...
- **颜色**：`rgb {...}`、`hsv {...}`、`hsv360 {...}`。
- **块**：`{ ... }`，内部可包含注释、属性、值与封装变量等。
- **封装变量引用**：`@name`。
- **内联数学**：`@[ <expr> ]`，支持 `+ - * / %`、一元正负号、绝对值 `|x|` 与括号。

封装变量（scripted variable）：

- 声明：`@<name> = <value>`，其中 `<value>` 可为布尔/整数/浮点/字符串/内联数学。
- 引用：`@<name>`。

参数（parameter）：

- 语法：`$name$` 或 `$name|argument$`。
- 位置：可用于封装变量名、封装变量引用名、键、字符串、内联数学。
...
> [!warning]
> 参数、参数条件与内联数学等属于进阶语法，通常仅在特定定义（如封装效果/触发器）中生效或被引擎评估。
...
> [!tip]
>插件可以检测本地化文件的文件编码问题，并且支持自动修正文件编码。
