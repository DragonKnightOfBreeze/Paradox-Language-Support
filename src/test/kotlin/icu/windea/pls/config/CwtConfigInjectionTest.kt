...
class CwtConfigInjectionTest : BasePlatformTestCase() {
    ...
    fun testInjection_appends_and_parentChain() {
        ...
        run {
            ...
            val v = values.single()
            assertEquals(ChronicleStrings.blockFolder, v.value)
    ...
    fun testInjection_deepRecursive_ignored() {
        ...
        // - 注入时 `deepCopyForInjection` 会调用 `CwtConfigManipulationService.deepCopyConfigs(configToInject, delegatedConfig)`
        ...
        // - 在 `injectConfigs(parentConfig, ...)` 里调用 `CwtConfigManipulationService.deepCopyConfigs(parentConfig)`
