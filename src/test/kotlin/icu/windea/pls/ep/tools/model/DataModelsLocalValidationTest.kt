...
class DataModelsLocalValidationTest {
    ...
    fun readDlcLoadJson_ifExists() {
        ...
        val model = JsonService.mapper.readValue(file.toFile(), DlcLoadJson::class.java)
        // 基本断言 + 更严格校验
        assert(model != null)
    ...
    fun readContentLoadJson_ifExists() {
        ...
        val model = JsonService.mapper.readValue(file.toFile(), ContentLoadJson::class.java)
    ...
    fun readPlaylistJson_ifExists() {
        ...
        val root = JsonService.mapper.readTree(file.toFile())
        ...
        if (isV3) {
            val model = JsonService.mapper.readValue(file.toFile(), LauncherJsonV3::class.java)
            ...
            val model = JsonService.mapper.readValue(file.toFile(), LauncherJsonV2::class.java)
