...
import icu.windea.pls.extension.diagram.settings.Eu5EventTreeDiagramSettings
...
class Eu5EventTreeDiagramProvider : ParadoxEventTreeDiagramProvider(ParadoxGameType.Eu5) {
    ...
    override fun getPresentableName() = PlsDiagramBundle.message("eventTree.name.eu5")
    ...
    override fun getDiagramSettings(project: Project) = project.service<Eu5EventTreeDiagramSettings>()
    ...
    class DataModel(
        ...
        override fun showNode(definition: ParadoxScriptDefinitionElement, settings: ParadoxDiagramSettings.State): Boolean {
            if (provider !is Eu5EventTreeDiagramProvider) return true
            if (settings !is Eu5EventTreeDiagramSettings.State) return true
