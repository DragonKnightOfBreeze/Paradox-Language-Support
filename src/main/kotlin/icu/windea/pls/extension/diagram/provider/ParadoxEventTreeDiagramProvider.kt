...
abstract class ParadoxEventTreeDiagramProvider(gameType: ParadoxGameType) : ParadoxDefinitionDiagramProvider(gameType) {
            ...
            return when(nodeElement) {
                is ParadoxScriptProperty -> {
                    when {
                        ...
                        nodeItem is PsiFile -> {
                            //事件图片
                            val iconUrl = ParadoxImageResolver.resolveUrlByFile(nodeItem.virtualFile, nodeElement.getUserData(PlsKeys.iconFrame) ?: 0)
    ...
    abstract class DataModel(
        ...
        override fun getModificationTracker() = ParadoxPsiModificationTracker.getInstance(project).ScriptFileTracker("events:txt")
    }
}