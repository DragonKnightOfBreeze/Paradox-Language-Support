 ...
 * （同一文件中）多个语言区域的检查。
 *
 * @property ignoredFileNames （配置项）需要忽略的文件名，以逗号分隔，忽略大小写。默认为"languages.yml"
 */
class MultipleLocalesInspection : LocalInspectionTool() {
	@OptionTag(converter = CommaDelimitedStringSetIgnoreCaseConverter::class)
	@JvmField var ignoredFileNames = "languages.yml".toCommaDelimitedStringSet(ignoreCase = true)
	
	override fun checkFile(file: PsiFile, manager: InspectionManager, isOnTheFly: Boolean): Array<out ProblemDescriptor>? {
		if(file !is ParadoxLocalisationFile) return null //不应该出现
		if(ignoredFileNames.contains(file.name)) return null //忽略
		...
		return panel {
			...
			row {
				expandableTextField({ it.toCommaDelimitedStringMutableList() }, { it.toCommaDelimitedString() })
					.bindText({ ignoredFileNames.toCommaDelimitedString() }, { ignoredFileNames = it.toCommaDelimitedStringSet(ignoreCase = true) })
					.applyToComponent {
						whenTextChanged {
							val document = it.document
							val text = document.getText(0, document.length)
							if(text != ignoredFileNames.toCommaDelimitedString()) ignoredFileNames = text.toCommaDelimitedStringSet(ignoreCase = true)
						}
					}
