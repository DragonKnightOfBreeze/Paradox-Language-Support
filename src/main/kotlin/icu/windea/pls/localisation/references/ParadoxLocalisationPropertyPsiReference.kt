...
class ParadoxLocalisationPropertyPsiReference(
    ...
    private object Resolver : ResolveCache.AbstractResolver<ParadoxLocalisationPropertyPsiReference, PsiElement> {
        override fun resolve(ref: ParadoxLocalisationPropertyPsiReference, incompleteCode: Boolean): PsiElement? {
            return ref.doResolve(ResolvedTypes.ALL)
        }
    }
    ...
    private object MultiResolver : ResolveCache.PolyVariantResolver<ParadoxLocalisationPropertyPsiReference> {
        override fun resolve(ref: ParadoxLocalisationPropertyPsiReference, incompleteCode: Boolean): Array<out ResolveResult> {
            return ref.doMultiResolve(ResolvedTypes.ALL)
        }
    }
    
    private object LocalisationResolver : ResolveCache.AbstractResolver<ParadoxLocalisationPropertyPsiReference, ParadoxLocalisationProperty> {
        override fun resolve(ref: ParadoxLocalisationPropertyPsiReference, incompleteCode: Boolean): ParadoxLocalisationProperty? {
            return ref.doResolve(ResolvedTypes.Localisation).castOrNull()
        }
    }
    ...
    fun resolveLocalisation(): ParadoxLocalisationProperty? {
        return ResolveCache.getInstance(project).resolveWithCaching(this, LocalisationResolver, false, false)
    }
    
    private fun doResolve(resolvedTypes: Int): PsiElement? {
        val element = element
        ...
        if(BitUtil.isSet(resolvedTypes, ResolvedTypes.Localisation)) {
        ...
        if(BitUtil.isSet(resolvedTypes, ResolvedTypes.Parameter)) {
            //尝试解析成parameter
    ...
    private fun doMultiResolve(resolvedTypes: Int): Array<out ResolveResult> {
        val element = element
    ...
    object ResolvedTypes {
        const val Localisation = 0x01
        const val Parameter = 0x02
        const val ALL = Localisation or Parameter
    }
}
