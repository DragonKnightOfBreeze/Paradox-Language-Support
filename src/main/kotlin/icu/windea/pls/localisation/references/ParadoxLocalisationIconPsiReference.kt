...
class ParadoxLocalisationIconPsiReference(
	...
	override fun resolve(exact: Boolean): PsiElement? {
		...
		val ddsFile = ParadoxFilePathSearch.search(iconName, project, ddsFileExpression, selector = fileSelector).find()
	...
	override fun multiResolve(incompleteCode: Boolean): Array<ResolveResult> {
		...
		val fileSelector = fileSelector().gameTypeFrom(element).preferRootFrom(element)
		val ddsFileExpression = CwtValueExpression.resolve("icon[gfx/interface/icons/]")
		val ddsFiles = ParadoxFilePathSearch.search(iconName, project, ddsFileExpression, selector = fileSelector).findAll()
