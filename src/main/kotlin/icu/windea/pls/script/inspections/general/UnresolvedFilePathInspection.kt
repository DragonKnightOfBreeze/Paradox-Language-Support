			...
			when(expression.type) {
				CwtDataType.AbsoluteFilePath -> {
					...
					if(VfsUtil.findFile(path, false) == null) {
						holder.registerProblem(location, PlsBundle.message("inspection.script.general.unresolvedFilePath.description.1", path), ProblemHighlightType.LIKE_UNKNOWN_SYMBOL,
				...
				CwtDataType.FilePath -> {
					...
					if(ParadoxFilePathSearch.search(filePath, project, selector = selector).findFirst() == null) {
						holder.registerProblem(location, PlsBundle.message("inspection.script.general.unresolvedFilePath.description.2", filePath), ProblemHighlightType.LIKE_UNKNOWN_SYMBOL,
							ImportGameOrModDirectoryFix(valueElement)
						)
					}
				}
				CwtDataType.Icon -> {
					...
					val selector = fileSelector().gameTypeFrom(valueElement)
					if(ParadoxFilePathSearch.search(filePath, project, selector = selector).findFirst() == null) {
						holder.registerProblem(location, PlsBundle.message("inspection.script.general.unresolvedFilePath.description.3", filePath), ProblemHighlightType.LIKE_UNKNOWN_SYMBOL,
							ImportGameOrModDirectoryFix(valueElement)
						)
					}
				}
