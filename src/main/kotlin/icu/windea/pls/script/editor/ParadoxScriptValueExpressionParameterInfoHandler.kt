 ...
 * 显示SV表达式的参数信息（如果有）。
 */
class ParadoxScriptValueExpressionParameterInfoHandler : ParameterInfoHandler<ParadoxExpressionElement, Set<String>> {
	//2. 光标位置位于带参数的SV表达式的参数部分中，即在value:xxx|xxx|xxx|的第一个管道符之后
	
	override fun findElementForParameterInfo(context: CreateParameterInfoContext): ParadoxExpressionElement? {
		val offset = context.offset
		val element = context.file.findElementAt(offset) ?: return null
		val targetElement = element.parent.castOrNull<ParadoxExpressionElement>() ?: return null
		...
		val definitions = findDefinitionsByType(svName, "script_value", context.project, selector = selector)
	...
	override fun findElementForUpdatingParameterInfo(context: UpdateParameterInfoContext): ParadoxExpressionElement? {
		val offset = context.offset
		val element = context.file.findElementAt(offset) ?: return null
		val targetElement = element.parent.castOrNull<ParadoxExpressionElement>() ?: return null
	...
	override fun updateParameterInfo(parameterOwner: ParadoxExpressionElement, context: UpdateParameterInfoContext) {
		context.parameterOwner = parameterOwner
	}
	
	override fun showParameterInfo(element: ParadoxExpressionElement, context: CreateParameterInfoContext) {
