...
class CwtInOnActionInjectedConfigProcessor : CwtInjectedConfigProcessor {
    ...
    override fun process(parentConfig: CwtMemberConfig<*>, config: CwtMemberConfig<*>, configExpression: CwtDataExpression): List<String>? {
        if (configExpression.type != CwtDataTypes.Definition) return null
        if (configExpression.expressionString != expression) return null
        ...
        logger.debug { "Applied config injection in declaration of on action `${onActionConfig.name}`: replace `$configExpression` with ${result.joinToString()}" }
        return result
    }
...
class CwtTechnologyWithLevelInjectedConfigProcessor : CwtInjectedConfigProcessor {
    ...
    override fun process(parentConfig: CwtMemberConfig<*>, config: CwtMemberConfig<*>, configExpression: CwtDataExpression): List<String>? {
        if (configExpression.type != CwtDataTypes.Definition) return null
        if (configExpression.expressionString !in expressions) return null
        val result = injectedExpressions
        logger.debug { "Applied config injection: replace `${configExpression}` with ${result.joinToString()}`" }
        return result
    }
}
