...
class ParadoxScriptDefinitionExpressionSupport : ParadoxScriptExpressionSupportBase() {
    ...
    override fun resolveAll(element: ParadoxExpressionElement, rangeInElement: TextRange?, expressionText: String, config: CwtConfig<*>, role: ParadoxExpressionRole): List<PsiElement> {
        val fullNames = CwtConfigManager.getFullNamesFromSuffixAware(config, expressionText)
        val configGroup = config.configGroup
        val project = configGroup.project
...
class ParadoxScriptAliasNameExpressionSupport : ParadoxScriptExpressionSupportBase() {
    ...
    override fun annotate(element: ParadoxExpressionElement, rangeInElement: TextRange?, expressionText: String, config: CwtConfig<*>, holder: AnnotationHolder) {
        ...
        ParadoxExpressionService.annotateScriptExpression(element, rangeInElement, expressionText, aliasConfig, holder)
    }
...
class ParadoxScriptPathReferenceExpressionSupport : ParadoxScriptExpressionSupportBase() {
    ...
    override fun resolveAll(element: ParadoxExpressionElement, rangeInElement: TextRange?, expressionText: String, config: CwtConfig<*>, role: ParadoxExpressionRole): List<PsiElement> {
        ...
        val project = configGroup.project
        if (configExpression.type == CwtDataTypes.AbsoluteFilePath) {
            return expressionText.toVirtualFile()?.toPsiFile(project).to.singletonListOrEmpty()
        } else {
            // if (ParadoxPathReferenceExpressionSupport.get(configExpression) == null) return null
            val pathReference = expressionText.normalizePath()
            val selector = ParadoxFilePathSearch.selector(project, element).contextSensitive()
