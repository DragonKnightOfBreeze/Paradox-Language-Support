...
class CwtBaseRelatedConfigProvider : CwtRelatedConfigProvider {
    override fun getRelatedConfigs(file: PsiFile, offset: Int): Collection<CwtConfig<*>> {
        ...
        // 基于所有匹配的规则
        run {
            val matchOptions = ParadoxMatchOptions(lenient = false, forExpression = true, forDeclarationRoot = true)
...
class CwtInComplexExpressionRelatedConfigProvider : CwtRelatedConfigProvider {
    override fun getRelatedConfigs(file: PsiFile, offset: Int): Collection<CwtConfig<*>> {
        ...
        val offsetInExpression = offset - textRange.startOffset - ParadoxExpressionService.getExpressionOffset(element)
    ...
    override fun getRelatedConfigs(file: PsiFile, offset: Int): Collection<CwtConfig<*>> {
        ...
        run {
            ...
            for (reference in element.references) {
                when {
                    ParadoxReferenceConstraint.Parameter.canResolve(reference) -> {
                    ...
                    ParadoxReferenceConstraint.ComplexEnumValue.canResolve(reference) -> {
                    ...
                    ParadoxReferenceConstraint.DynamicValueReference.canResolve(reference) -> {
