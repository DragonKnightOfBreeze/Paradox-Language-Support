...
interface ParadoxPathReferenceExpression {
    ...
    companion object INSTANCE {
        @JvmStatic val EP_NAME = ExtensionPointName.create<ParadoxPathReferenceExpression>("icu.windea.pls.paradoxPathReferenceExpression")
        
        fun matches(configExpression: CwtDataExpression, filePath: String, ignoreCase: Boolean = true): Boolean {
            return EP_NAME.extensions.any action@{
                if(it.supports(configExpression)) return@action false 
                it.matches(configExpression, filePath, ignoreCase)
            }
        }
        
        fun extract(configExpression: CwtDataExpression, filePath: String, ignoreCase: Boolean = true): String? {
            return EP_NAME.extensions.firstNotNullOfOrNull action@{
                if(it.supports(configExpression)) return@action null
                it.extract(configExpression, filePath, ignoreCase)
            }
        }
    }
}
