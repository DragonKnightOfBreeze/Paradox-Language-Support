...
object ParadoxScopeCallStatementManipulationService {
    ...
    fun getExistsProperty(element: ParadoxScriptProperty): ParadoxScriptProperty? {
        if (element.propertyKey.text == "exists") {
        ...
        while (prev != null) {
            if (prev is ParadoxScriptProperty) {
                if (prev.propertyKey.text == "exists" && matchesExistsValue(prev, element)) return prev
                break
            }
    ...
    fun collectPrecedingComments(element: ParadoxScriptProperty): List<PsiComment> {
        val comments = mutableListOf<PsiComment>()
        var current = element.prevSibling
        while (current != null && current !is ParadoxScriptProperty) {
            if (current is PsiComment) comments.add(current)
    ...
    fun convertToSafeForm(element: ParadoxScriptProperty, project: Project, gameType: ParadoxGameType) {
        ...
        val key = secondProperty.propertyKey.text
        ...
        val anchor = existsProperty.prevSibling
        // 从最后一个注释开始移动，确保顺序正确
        ...
        if (firstToDelete != null && firstToDelete !== secondProperty && secondProperty !== existsProperty) {
            parent.deleteChildRange(firstToDelete, secondProperty)
        } else if (secondProperty !== existsProperty) {
            // 直接删除 secondProperty（两者相邻，中间无内容）
            secondProperty.delete()
        }

        // 构建安全形式属性文本（仅属性本身）
        val safeText = key + safeSeparator + valueText
    ...
    fun convertToNormalForm(element: ParadoxScriptProperty, project: Project, gameType: ParadoxGameType) {
        val key = element.propertyKey.text
        ...
        // 创建 exists 属性和第二属性
        val firstText = "exists = " + key
        val secondText = key + " = " + valueText
...
private fun isExistsPropertyOfNormalForm(element: ParadoxScriptProperty): Boolean {
    if (element.propertyKey.text != "exists") return false
...
private fun matchesExistsValue(existsProperty: ParadoxScriptProperty, targetProperty: ParadoxScriptProperty): Boolean {
    val existsValue = existsProperty.propertyValue ?: return false
    val existsValueText = when (val child = existsValue.firstChild) {
        is ParadoxScriptString -> child.value ?: child.text
        else -> existsValue.text
    }
    val targetKey = targetProperty.propertyKey.text
    val targetKeyName = targetProperty.propertyKey.name
    return existsValueText == targetKey || existsValueText == targetKeyName
}

private fun findPreviousProperty(element: ParadoxScriptProperty): ParadoxScriptProperty? {
    var prev = element.prevSibling
    while (prev != null) {
        if (prev is ParadoxScriptProperty) return prev
        prev = prev.prevSibling
    }
    return null
}
...
private fun findExistsPropertyBefore(element: ParadoxScriptProperty): ParadoxScriptProperty? {
    ...
    while (prev != null) {
        if (prev is ParadoxScriptProperty) {
            if (prev.propertyKey.text == "exists") return prev
            break
        }
        prev = prev.prevSibling
    }
