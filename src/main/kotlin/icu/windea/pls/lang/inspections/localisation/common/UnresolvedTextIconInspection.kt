...
class UnresolvedTextIconInspection : LocalInspectionTool() {
    object Constants {
        const val definitionType = ParadoxDefinitionTypes.textIcon
    }
    ...
    override fun getOptionsPane(): OptPane {
        return OptPane.pane(
            OptPane.checkbox("ignoredNames", ChronicleInspectionBundle.message("inspection.localisation.unresolvedTextIcon.option.ignoredNames")),
    ...
    private fun check(element: ParadoxLocalisationTextIcon, configGroup: CwtConfigGroup, holder: ProblemsHolder) {
        val name = element.name ?: return
        if (skip(name, element, configGroup)) return // 忽略
        val reference = element.reference
        if (reference == null || reference.resolve() != null) return
        val location = element.idElement ?: return
        val description = ChronicleInspectionBundle.message("inspection.localisation.unresolvedTextIcon.desc", name)
        holder.registerProblem(location, description, ProblemHighlightType.LIKE_UNKNOWN_SYMBOL)
    }

    private fun skip(name: String, element: ParadoxLocalisationTextIcon, configGroup: CwtConfigGroup): Boolean {
