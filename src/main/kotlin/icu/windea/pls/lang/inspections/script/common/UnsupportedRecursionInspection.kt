...
class UnsupportedRecursionInspection : LocalInspectionTool(), DumbAware {
    ...
    override fun buildVisitor(holder: ProblemsHolder, isOnTheFly: Boolean): PsiElementVisitor {
        return object : ParadoxScriptVisitor() {
            override fun visitScriptedVariable(element: ParadoxScriptScriptedVariable) {
                ...
                checkScriptedVariable(element, holder)
            }

            override fun visitProperty(element: ParadoxScriptProperty) {
                ProgressManager.checkCanceled()
                checkDefinition(element, holder)
            }
        }
    }

    private fun checkScriptedVariable(element: ParadoxScriptScriptedVariable, holder: ProblemsHolder) {
        ...
        val description = ChronicleInspectionBundle.message("script.unsupportedRecursion.desc.1")
    ...
    private fun checkDefinition(element: ParadoxScriptProperty, holder: ProblemsHolder) {
        ...
        val description = when {
            definitionInfo.type == "scripted_trigger" -> ChronicleInspectionBundle.message("script.unsupportedRecursion.desc.2")
            definitionInfo.type == "scripted_effect" -> ChronicleInspectionBundle.message("script.unsupportedRecursion.desc.3")
            else -> return
        }
