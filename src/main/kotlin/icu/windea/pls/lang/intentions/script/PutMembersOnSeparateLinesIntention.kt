...
/**
 * 将成员放到不同的行。适用于 [ParadoxScriptBlock] 和 [ParadoxScriptNormalConditionalBlock]。
 *
 * ```paradox_script
 * # before
 * { V K = V }
    ...
    override fun invoke(context: ActionContext, element: ParadoxScriptBoundMemberContainer, updater: ModPsiUpdater) {
        ...
        val newElement = when (element) {
            is ParadoxScriptNormalConditionalBlock -> {
                ...
                ParadoxScriptElementFactory.createNormalConditionalBlockFromText(context.project, newText)
            }
            else -> {
                val newText = "{\n${membersText}\n}"
