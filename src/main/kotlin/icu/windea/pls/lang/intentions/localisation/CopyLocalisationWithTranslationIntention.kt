...
class CopyLocalisationWithTranslationIntention : ManipulateLocalisationIntentionBase.WithLocalePopup(), DumbAware {
    override fun getFamilyName() = ChronicleIntentionBundle.message("intention.copyLocalisationWithTranslation")
    ...
    override suspend fun doHandle(project: Project, file: PsiFile, context: Context) {
        ...
        withBackgroundProgress(project, ChronicleIntentionBundle.message("intention.copyLocalisationWithTranslation.progress.title", selectedLocale.text)) action@{
            ...
            runCatchingCancelable r@{
                ...
                reportProgressScope(contextsToHandle.size) { reporter ->
                    contextsToHandle.forEachConcurrent f@{ context ->
                        reporter.itemStep(ChronicleBundle.message("manipulation.localisation.translate.progress.itemStep", context.key)) {
    ...
    private fun createNotification(selectedLocale: CwtLocaleConfig, error: Throwable?): Notification {
        if (error == null) {
            val content = ChronicleIntentionBundle.message("intention.copyLocalisationWithTranslation.notification", selectedLocale.text, Messages.success())
        ...
        val content = ChronicleIntentionBundle.message("intention.copyLocalisationWithTranslation.notification", selectedLocale.text, Messages.failed()) + errorDetails
