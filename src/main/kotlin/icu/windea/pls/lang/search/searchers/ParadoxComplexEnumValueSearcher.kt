...
class ParadoxComplexEnumValueSearcher : QueryExecutorBase<ParadoxComplexEnumValueIndexInfo, ParadoxComplexEnumValueSearch.Parameters>() {
    ...
    private fun createActualKey(queryParameters: ParadoxComplexEnumValueSearch.Parameters): String {
        val type = queryParameters.enumName
        return PlsIndexUtil.createTypeKey(type)
    }

    private fun processInfo(
        queryParameters: ParadoxComplexEnumValueSearch.Parameters,
        file: VirtualFile,
    ...
    private fun matchesName(queryParameters: ParadoxComplexEnumValueSearch.Parameters, info: ParadoxComplexEnumValueIndexInfo): Boolean {
        if (queryParameters.name == null) return true
        return queryParameters.name.equals(info.name, info.caseInsensitive) // # 261
    }

    private fun matchesEnumName(queryParameters: ParadoxComplexEnumValueSearch.Parameters, info: ParadoxComplexEnumValueIndexInfo): Boolean {
