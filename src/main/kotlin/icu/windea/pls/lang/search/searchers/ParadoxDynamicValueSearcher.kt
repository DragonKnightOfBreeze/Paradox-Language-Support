...
class ParadoxDynamicValueSearcher : QueryExecutorBase<ParadoxDynamicValueIndexInfo, ParadoxDynamicValueSearch.Parameters>() {
    ...
    private fun processInfo(
        queryParameters: ParadoxDynamicValueSearch.Parameters,
        file: VirtualFile,
    ...
    private fun matchesName(queryParameters: ParadoxDynamicValueSearch.Parameters, info: ParadoxDynamicValueIndexInfo): Boolean {
        if (queryParameters.name == null) return true
        return queryParameters.name == info.name
    }

    private fun matchesType(queryParameters: ParadoxDynamicValueSearch.Parameters, info: ParadoxDynamicValueIndexInfo): Boolean {
        return queryParameters.types.contains(info.dynamicValueType)
    }
}
