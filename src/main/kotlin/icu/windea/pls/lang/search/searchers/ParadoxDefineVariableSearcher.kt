 ...
 * 定值命名空间的查询器。
 */
class ParadoxDefineVariableSearcher : QueryExecutorBase<ParadoxScriptProperty, Parameters>() {
    ...
    private fun processQuery(context: Context, consumer: Processor<in ParadoxScriptProperty>) {
        if (context.namespace != null && context.variable != null) {
            ...
            PlsIndexService.processElements(PlsIndexKeys.DefineVariable, key, context.project, context.scope) { element ->
                consumer.process(element)
            }
        } else if (context.namespace != null) {
            PlsIndexService.processElementsByKeys(PlsIndexKeys.DefineVariable, context.project, context.scope, { it.namespace == context.namespace }) { _, element ->
                consumer.process(element)
            }
        } else if (context.variable != null) {
            PlsIndexService.processElementsByKeys(PlsIndexKeys.DefineVariable, context.project, context.scope, { it.variable == context.variable }) { _, element ->
                consumer.process(element)
            }
        } else {
            PlsIndexService.processElementsByKeys(PlsIndexKeys.DefineVariable, context.project, context.scope) { _, element ->
                consumer.process(element)
            }
        }
    }
