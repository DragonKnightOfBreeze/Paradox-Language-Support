...
class ParadoxParameterSearcher : QueryExecutorBase<ParadoxParameterIndexInfo, ParadoxParameterSearch.SearchParameters>() {
    override fun processQuery(queryParameters: ParadoxParameterSearch.SearchParameters, consumer: Processor<in ParadoxParameterIndexInfo>) {
        ...
        if (contextKey.startsWith("inline_script@")) {
            ...
            if (name != null) {
                val props = StubIndex.getElements(ParadoxIndexKeys.InlineScriptArgumentByName, name, project, scope, ParadoxScriptProperty::class.java)
                publishInlineArgs(props, name, contextKey, gameType, consumer)
            } else {
                val keys = StubIndex.getInstance().getAllKeys(ParadoxIndexKeys.InlineScriptArgumentByName, project)
                for (key in keys) {
                    val props = StubIndex.getElements(ParadoxIndexKeys.InlineScriptArgumentByName, key, project, scope, ParadoxScriptProperty::class.java)
        ...
        doProcessFiles(scope) p@{ file ->
            ...
            val gt = selectGameType(file)
            if (gameType != null && gt != gameType) return@p true //check game type at file level
