...
class ParadoxSubstringTextBasedTargetSearcher : ParadoxTextBasedTargetSearcher() {
    override fun process(context: Context, consumer: Processor<in PsiElement>) {
        val inputText = context.queryParameters.text
        val snippets = getSnippets(inputText)
    ...
    private fun getSnippets(inputText: String): List<String> {
        // 按空白分隔
        return inputText.splitByBlank()
    }
