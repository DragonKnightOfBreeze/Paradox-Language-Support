...
class ParadoxDefinitionSearcher : QueryExecutorBase<ParadoxDefinitionIndexInfo, ParadoxDefinitionSearch.Parameters>() {
    ...
    private fun createActualKey(queryParameters: ParadoxDefinitionSearch.Parameters, constraint: ParadoxDefinitionIndexConstraint?): String {
    ...
    private fun processInfo(
        queryParameters: ParadoxDefinitionSearch.Parameters,
        file: VirtualFile,
    ...
    private fun matchesName(queryParameters: ParadoxDefinitionSearch.Parameters, info: ParadoxDefinitionIndexInfo, constraint: ParadoxDefinitionIndexConstraint?): Boolean {
        if (queryParameters.name == null) return true
        val ignoreCase = constraint?.ignoreCase == true
        return queryParameters.name.equals(info.name, ignoreCase)
    }

    private fun matchesType(queryParameters: ParadoxDefinitionSearch.Parameters, info: ParadoxDefinitionIndexInfo): Boolean {
        if (queryParameters.type == null) return true
        return queryParameters.type == info.type
    }

    private fun matchesSubtypes(queryParameters: ParadoxDefinitionSearch.Parameters, info: ParadoxDefinitionIndexInfo): Boolean {
