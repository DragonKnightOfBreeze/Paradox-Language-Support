 ...
 */
class BrowseSpecialPathsDialog(
    project: Project?,
    ...
    private fun copyAll() {
        val tuples = providers.mapNotNull { provider ->
            val path = provider.getPath(selectedFile, selectedGameType) ?: return@mapNotNull null
            val name = provider.text
            name to path.toString()
        }
        if (tuples.isEmpty()) return
        val maxNameLength = tuples.maxOfOrNull { (name) -> name.length } ?: 0
        val text = buildString {
            for ((name, path) in tuples) {
                append(name.padEnd(maxNameLength, ' ')).append("  ").append(path).appendLine()
            }
        }
