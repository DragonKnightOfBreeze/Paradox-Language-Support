...
object ParadoxConfigMatchService {
    ...
    fun matchesSubtype(context: CwtSubtypeConfigMatchContext, element: ParadoxDefinitionElement, subtypeConfig: CwtSubtypeConfig): Boolean {
        val fastResult = matchesSubtypeFast(context, subtypeConfig)
        if (fastResult != null) return fastResult
        ...
        return matchesDefinitionForSubtype(context, element, elementConfig)
    }

    fun matchesSubtypeFast(context: CwtSubtypeConfigMatchContext, subtypeConfig: CwtSubtypeConfig): Boolean? {
    ...
    private fun matchesDefinitionForSubtype(context: CwtSubtypeConfigMatchContext, definition: ParadoxDefinitionElement, propertyConfig: CwtPropertyConfig): Boolean {
        ...
        // 也不要基于参数化块表达式中的声明结构
        if (propertyConfig.configs.orEmpty().isEmpty()) return true
        val block = definition.block
        // 匹配值列表
    ...
    private fun matchesPropertyForSubtype(context: CwtSubtypeConfigMatchContext, definition: ParadoxDefinitionElement, property: ParadoxScriptProperty, propertyConfig: CwtPropertyConfig): Boolean {
        ...
        when {
            ...
            propertyConfig.stringValue != null -> {
                ...
                val nextContext = ParadoxExpressionMatchContext(propValue, expression, configGroup, context.options)
                return ParadoxExpressionMatchService.matchScriptExpression(nextContext, propertyConfig.valueExpression, propertyConfig).get(context.options)
            }
            // 匹配 alias
            ...
            CwtConfigMatchService.isSingleAliasEntry(propertyConfig) -> {
                return matchesSingleAliasForSubtype(context, definition, property, propertyConfig)
            }
            else -> {
                if (propertyConfig.configs.orEmpty().isEmpty()) return true
    ...
    private fun matchesPropertiesForSubtype(context: CwtSubtypeConfigMatchContext, definition: ParadoxDefinitionElement, block: ParadoxScriptMemberContainer?, propertyConfigs: List<CwtPropertyConfig>): Boolean {
        ...
        val matched = definition.properties(context.inline).all p@{ propertyElement ->
            ...
            val nextContext = ParadoxExpressionMatchContext(keyElement, expression, configGroup, context.options)
            val propConfigs = propertyConfigs.filterFast { config ->
                ParadoxExpressionMatchService.matchScriptExpression(nextContext, config.keyExpression, config).get(context.options)
            }
    ...
    private fun matchesValuesForSubtype(context: CwtSubtypeConfigMatchContext, block: ParadoxScriptMemberContainer?, valueConfigs: List<CwtValueConfig>): Boolean {
        ...
        val matched = block.values(context.inline).process p@{ valueElement ->
            ...
            val nextContext = ParadoxExpressionMatchContext(valueElement, expression, configGroup, context.options)
            val matched = valueConfigs.anyFast { config ->
                val matched = ParadoxExpressionMatchService.matchScriptExpression(nextContext, config.valueExpression, config).get(context.options)
    ...
    private fun matchesSingleAliasForSubtype(context: CwtSubtypeConfigMatchContext, definition: ParadoxDefinitionElement, property: ParadoxScriptProperty, propertyConfig: CwtPropertyConfig): Boolean {
        val configGroup = propertyConfig.configGroup
        val singleAliasName = propertyConfig.valueExpression.metadata.value?.orNull() ?: return false
