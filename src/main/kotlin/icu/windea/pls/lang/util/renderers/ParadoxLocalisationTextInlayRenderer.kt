...
class ParadoxLocalisationTextInlayRenderer(val hintsContext: ParadoxHintsContext) : ParadoxLocalisationTextRenderer<ParadoxLocalisationTextInlayRenderer.Scope, InlayPresentation?>() {
    var color: Color? = null

    fun withColor(color: Color?) = apply { this.color = color }

    override fun createScope(): Scope {
        return Scope(hintsContext, color)
    }
    ...
    class Scope(
    ...
    ) : ParadoxLocalisationTextRenderer.Scope<InlayPresentation?>() {
        private val truncateRemain: AtomicInteger = AtomicInteger(settings.localisationTextLengthLimit) // 记录到需要截断为止所剩余的长度
        ...
        val settings: ParadoxHintsSettings get() = hintsContext.settings
        ...
        override fun renderIcon(element: ParadoxLocalisationIcon) {
            ...
            runCatchingCancelable r@{
                ...
                if (originalIconHeight > settings.iconHeightLimit) return@r // 图标过大，不再尝试渲染
                // 基于内嵌提示的字体大小缩放图标，直到图标宽度等于字体宽度
        ...
        private fun truncatedSmallText(text: String): InlayPresentation {
            val limit = settings.localisationTextLengthLimit
