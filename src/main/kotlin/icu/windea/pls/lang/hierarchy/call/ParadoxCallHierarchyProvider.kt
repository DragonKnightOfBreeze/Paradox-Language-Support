...
class ParadoxCallHierarchyProvider : HierarchyProvider {
    override fun getTarget(dataContext: DataContext): PsiElement? {
        ...
        run {
            ...
            ParadoxPsiFileService.findScriptedVariable(file, offset) { DEFAULT or BY_REFERENCE }?.let { return it }
            ParadoxPsiFileService.findDefinition(file, offset) { DEFAULT or BY_REFERENCE }?.let { return it }
            ParadoxPsiFileService.findLocalisation(file, offset) { DEFAULT or BY_REFERENCE }?.let { return it }
        }
        return null
    }
