...
/**
 * 通过内嵌提示显示定义引用的提示文本。
 * 来自显示名称（即最相关的本地化文本），或者对应的扩展规则。优先级从低到高。
 *
...
class ParadoxDefinitionReferenceHintTextHintsProvider : ParadoxHintsProvider() {
    ...
    override val name get() = ChronicleBundle.message("script.hints.definitionReferenceHintText")
    override val description get() = ChronicleBundle.message("script.hints.definitionReferenceHintText.description")
    ...
    override fun collectFromElement(element: PsiElement, sink: InlayHintsSink) {
        ...
        val primaryLocalisation = ParadoxCodeInsightService.getHintLocalisation(resolved) ?: return
