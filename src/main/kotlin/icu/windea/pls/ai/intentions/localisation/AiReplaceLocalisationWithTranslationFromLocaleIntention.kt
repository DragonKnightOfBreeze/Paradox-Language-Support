...
class AiReplaceLocalisationWithTranslationFromLocaleIntention : ManipulateLocalisationIntentionBase.WithLocalePopupAndPopup<String>(), DumbAware {
    override fun getFamilyName() = PlsAiBundle.message("ai.intention.replaceLocalisationWithTranslationFromLocale")
    ...
    private fun createNotification(selectedLocale: CwtLocaleConfig, error: Throwable?, withWarnings: Boolean): Notification {
        if (error == null) {
            if (!withWarnings) {
                val content = PlsAiBundle.message("ai.intention.replaceLocalisationWithTranslationFromLocale.notification", selectedLocale.text, Messages.success())
                return PlsNotificationGroups.manipulation().createNotification(content, NotificationType.INFORMATION)
            }
            val content = PlsAiBundle.message("ai.intention.replaceLocalisationWithTranslationFromLocale.notification", selectedLocale.text, Messages.partialSuccess())
        ...
        val content = PlsAiBundle.message("ai.intention.replaceLocalisationWithTranslationFromLocale.notification", selectedLocale.text, Messages.partialSuccess()) + errorDetails
        return PlsNotificationGroups.manipulation().createNotification(content, NotificationType.WARNING)
    }
}
