...
class AiReplaceLocalisationWithTranslationIntention : ManipulateLocalisationIntentionBase.WithLocalePopupAndPopup<String>(), DumbAware {
    override fun getFamilyName() = ChronicleAiBundle.message("intention.replaceLocalisationWithTranslation")
    ...
    override suspend fun doHandle(project: Project, file: PsiFile, context: Context<String>) {
        ...
        withBackgroundProgress(project, ChronicleAiBundle.message("intention.replaceLocalisationWithTranslation.progress.title", selectedLocale.text)) action@{
    ...
    private fun createNotification(selectedLocale: CwtLocaleConfig, error: Throwable?, withWarnings: Boolean): Notification {
        if (error == null) {
            if (!withWarnings) {
                val content = ChronicleAiBundle.message("intention.replaceLocalisationWithTranslation.notification", selectedLocale.text, Messages.success())
                return ChronicleNotificationGroups.manipulation().createNotification(content, NotificationType.INFORMATION)
            }
            val content = ChronicleAiBundle.message("intention.replaceLocalisationWithTranslation.notification", selectedLocale.text, Messages.partialSuccess())
        ...
        val content = ChronicleAiBundle.message("intention.replaceLocalisationWithTranslation.notification", selectedLocale.text, Messages.partialSuccess()) + errorDetails
        return ChronicleNotificationGroups.manipulation().createNotification(content, NotificationType.WARNING)
    }
}
