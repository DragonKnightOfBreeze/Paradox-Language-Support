...
object PlsPromptManager {
    private val velocityEngine by lazy {
        ...
        VelocityEngine(properties).apply { init() }
    }

    /**
     * 从 Velocity 模版 [Template] 生成提示文本。
     *
     * @param name 名称。位于 `prompts` 目录下，去除可能的语言区域后缀以及扩展名后的文件名。
    ...
    private fun getTemplate(name: String): Template {
        val prefix = "prompts/"
        val suffix = getLocaleSuffix()
        val templateName = "$prefix$name$suffix"
        return velocityEngine.getTemplate(templateName)
    }

    private fun getLocaleSuffix(): String {
        return if (DynamicBundle.getLocale() == Locale.SIMPLIFIED_CHINESE) "_zh" else ""
    }
}
