...
class PlsAiTranslateLocalisationService : PlsAiManipulateLocalisationService() {
    ...
    fun createDescriptionPopup(project: Project, callback: (String) -> Unit): JBPopup {
        ...
        val panel = panel {
            row {
                cell(textField).align(AlignX.FILL).columns(COLUMNS_LARGE).focused()
                    .comment(PlsBundle.message("manipulation.localisation.translate.popup.comment"), MAX_LINE_LENGTH_WORD_WRAP)
            }
        }
        val popup = JBPopupFactory.getInstance()
            ...
            .setTitle(PlsBundle.message("manipulation.localisation.translate.popup.title"))
            .createPopup()
