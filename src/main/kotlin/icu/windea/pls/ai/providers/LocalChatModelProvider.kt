    ...
    private suspend fun checkApiVersion(baseUrl: String, ref: AtomicReference<StatusResult>) {
        ...
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                val r = StatusResult(false, ChronicleAiBundle.message("test.error.title"), ChronicleAiBundle.message("test.error.local.unreachable", versionUrl))
                ref.set(r)
            }
        }
    }
    ...
    private suspend fun checkApiTags(options: Options, baseUrl: String, ref: AtomicReference<StatusResult>) {
        ...
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                val r = StatusResult(false, ChronicleAiBundle.message("test.error.title"), ChronicleAiBundle.message("test.error.local.modelMissing", options.modelName))
                ref.set(r)
            }
        }
    }
