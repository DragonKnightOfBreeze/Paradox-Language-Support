<<<<<<<< HEAD:documents/ai-notes/domain/规则系统_规则.md
# 规则系统_规则
========
# 领域：规则系统 规则
>>>>>>>> dev/3.0.1-dev:documents/ai-notes/domain/领域_规则系统_规则.md
...
- 文档与指南：
  ...
  - CWT 指引（CWTools）：`references/documents/guidance.md)`
...
- `CwtConfig<T : PsiElement>`：规则统一抽象。
  - 关键属性：
    ...
    - `configExpression: CwtDataExpression?`：绑定的数据表达式（默认 null，由不同实现重载）。
...
- `CwtMemberConfig<T : CwtMemberElement>`：成员规则（属性或值）。
  ...
  - 典型扩展：
    ...
    - 附加元信息：`originalConfig`（被重载前的原始规则）、`overriddenProvider`（重载提供者）、`declarationConfigContext` 与 `declarationConfigCacheKey`（声明规则上下文）。
...
> 详细指南：`references/documents/guidance.md)`；PLS 具体扩展请对照 `docs/zh/config.md` 的“编写规范（扩展的 CWT 规则）”。
...
- PLS 扩展字段：
  - `originalConfig`：保留被重载前的原始规则
  - `overriddenProvider`：记录重载来源（远程/本地/项目本地等）
- 典型流程：
  1) 读取内置规则分组 → 读取本地/项目的本地规则分组
  2) 根据路径与 ID 进行合并/覆盖
