<<<<<<<< HEAD:documents/ai-notes/domain/模组开发_作用域.md
# 模组开发_作用域
========
# 领域：模组开发 作用域
>>>>>>>> dev/3.0.1-dev:documents/ai-notes/domain/领域_模组开发_作用域.md
...
- **作用域（scope）** 又称 **范围**，是蠢驴脚本（事件/触发器/效果等）描述语义的“主语”。
...
一切触发器（trigger）与效果（effect）都在“一个具体对象”上执行，这个对象就是当前作用域（scope）。作用域是“主语”，不是类型名；它通过事件的默认 ROOT、系统作用域（THIS/ROOT/FROM/PREV）、以及遍历/流程/属性跳跃被确定与传递。
...
对象的静态类别（如 `country`、`planet`、`ship` 等），决定可用的触发器/效果集合。可用 `is_scope_type = <type>` 检测 `THIS` 是否为该类型。类型名很少直接写在脚本里，通常由上下文或属性跳跃隐式确定。
