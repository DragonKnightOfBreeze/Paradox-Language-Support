<<<<<<<< HEAD:documents/ai-notes/domain/模组导入导出.md
# 模组导入导出
========
# 领域：模组导入导出
>>>>>>>> dev/3.0.1-dev:documents/ai-notes/domain/领域_模组导入导出.md
...
- 插件架构建议
  - 主插件（Core）
    - 定义 EP：例如 `com.your.plugin.ep.ModImporter`, `ModExporter`
...
- 基于 IronyModManager（IMM）源码核对：V2 的 `playlist.json` 中 `mods`].position` 使用的是“十六进制字符串，固定 10 位，小写，前导 0 填充”，计算方式为 [(index + 1 + 4096).ToString("x10")`。
...
- V2 JSON 导出（`src/IronyModManager.IO/Mods/Exporter/ParadoxLauncherExporter.cs`）
  ...
  - 同时排除 `ModSource.Local` 的模组（仅导出 pdx/steam）
...
- V2 JSON 导出（`icu.windea.pls.ep.tools.exporter.ParadoxLauncherJsonV2Exporter`）
  - 通过 `ParadoxMetadataManager.formatLauncherPosition(index, isV4Plus=false)` 生成 position
...
- V2 JSON 导入（`icu.windea.pls.ep.tools.importer.ParadoxLauncherJsonImporter` 以及工具方法）
  - 当前 `parseLauncherV2PositionToInt``
...
- `ParadoxGameJsonExporter`
  - 已复用 `ParadoxMetadataManager.buildDescriptorMapping`，并在路径映射失败时回退到 `mod/ugc_{id}.mod`，行为合理
...
- 修改工具方法 `ParadoxMetadataManager.formatLauncherPosition`
  - 逻辑：
    - `isV4Plus == true` → `index.toString()`（整数）
    - `isV4Plus == false`（V2）→ `(index + 4096 + 1).toString(16).padStart(10, '0')`
- 修改 `ParadoxMetadataManager.parseLauncherV2PositionToInt`
  - 逻辑（伪码）：
    ```kotlin
...
- 修正模型注释与测试数据
  - `JsonModels.kt` 中 V2 `position` 注释改回十六进制
  - `src/test/resources/tools/playlist_v2.json` 的 `position` 改为十六进制样例（如：`0000001001` 对应 4097 → 0x1001）
...
- 后续建议
  - 若你同意，我可以直接提交一个最小改动 PR：
    - 修改 `formatLauncherPosition](cci:1://file:///D:/Documents/Projects/__Pinned/Paradox-Language-Support/src/main/kotlin/icu/windea/pls/lang/util/ParadoxMetadataManager.kt:196:4-204:5)、[parseLauncherV2PositionToInt`
    - 调整 V2 测试样例与断言
  - 如需同时兼容“V2 DB TEXT position”的历史格式，请提供一份旧版 `launcher-v2.sqlite` 样例；我将基于样例确定 TEXT 的真实编码（十进制还是十六进制），再给出精确实现。

我参考了你的“去除 sqlite 依赖”的记录与会话笔记，以及之前对“导入/导出器补充中文 KDoc、SQLite 自动化测试均已通过”的记忆，并结合本机数据库与 IMM 源码完成以上核查。需要我现在就落实上述小改动吗？
### 结论速览

- Launcher JSON V2 的 `mods`].position` 在 IronyModManager（IMM）中是“十六进制字符串（小写）+ 左侧补零到 10 位”，计算为 [(index + 4096 + 1).ToString("x10")`。证据来自 `IronyModManager.IO/Mods/Exporter/ParadoxLauncherExporter.cs`。
- 你本机的 `launcher-v2.sqlite` 实测为 V4+ 模式：`playsets_mods.position` 的类型为 INTEGER，且数据为从 0 递增的整数（通过 MCP 工具实时读取确认，`knex_migrations` 含 `modifyPositionToInteger`）。
...
- 本地数据库（通过 MCP: `jdbc_launcher_v2_sqlite`）
  ...
  - `select playsetId, modId, enabled, position from playsets_mods ...`：position 为 0、1、2… 递增
...
- 导出 Launcher JSON V2（`icu.windea.pls.ep.tools.exporter.ParadoxLauncherJsonV2Exporter`）
  - PLS：`position` 由 `ParadoxMetadataManager.formatLauncherPosition(index, false)` 生成，目前是“十进制字符串（10 位补零）”
...
- 导入 Launcher JSON V2（`icu.windea.pls.ep.tools.importer.ParadoxLauncherJsonImporter`）
  - PLS：版本探测很精准（看 `mods`].position` 类型），排序解析仅按十进制（[parseLauncherV2PositionToInt`）
...
- 导出 Game JSON（`icu.windea.pls.ep.tools.exporter.ParadoxGameJsonExporter`）
  - PLS：使用 `buildDescriptorMapping` + `remoteId` 回退 `mod/ugc_{id}.mod`，逻辑稳妥
  - IMM：类似策略
...
- 修改工具方法（`icu.windea.pls.lang.util.ParadoxMetadataManager`）
  - `formatLauncherPosition(index, isV4Plus)`：
    - `isV4Plus == true` → `index.toString()`（整数）
    - `isV4Plus == false` → `(index + 4097).toString(16).padStart(10, '0')`（十六进制 10 位，小写）
  - `parseLauncherV2PositionToInt(pos: String?)`：
...
- 调整 V2 测试样例与注释
  - `src/test/resources/tools/playlist_v2.json`：
    - 将 `position` 修改为十六进制 10 位（如 4097 → 0x1001 → `0000001001`）
  - `JsonModels.kt` 中 V2 的注释同步改为十六进制说明
