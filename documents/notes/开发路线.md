...
- [x] P2 分析与优化构建索引时的性能和内存占用
  - 参见：[性能分析](analysis/性能分析.md#task-1)
...
- [ ] P1 优化：对于频繁调用的服务类，考虑不直接声明为 `@Service`，而是另外提供 `@Service class LifecycleService`。这种场合下，调用 `getInstance()` 可能积累数秒的耗时
- [ ] P1 优化：将 `CwtConfigManipulationService.expandConfigExpression` 改为 processor 模式，必要时再转为序列，以优化性能
- [ ] P1 优化：通过添加 Kotlin 编译器参数（`-Xno-param-assertions` `-Xno-call-assertions` `-Xno-receiver-assertions`）以禁用非空检查（`checkNotNullParameter(arg, "argName")`）
