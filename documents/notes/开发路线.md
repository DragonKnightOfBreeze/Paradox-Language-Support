...
细微的BUG修复、功能完善和代码重构，完善测试基础设施，性能与内存的分析与优化。
...
- [x] P2 优化：为合并索引提供专门的上下文对象，直接在其中缓存 expressionReferences，并且跳过不需要的逻辑，重构相关代码
  - 参见：[性能分析](analysis/性能分析.md#a6)
- [x] P1 优化：在 `ParadoxFilePathSearch` 中额外提供限定了文件扩展名的查询方法，以优化查询性能
  - 参见：[性能分析](analysis/性能分析.md#b1)
- [x] P2 优化：为 `ParadoxCompositeLocalisationIconSupport.fromDefinition` 中注册过的定义类型提供单独的索引约束
  - 参见：[性能分析](analysis/性能分析.md#b1)
- [x] P2 优化：为定义和本地化提供单独的、对应已有的索引约束（`indexConstraint`）的受限索引（`constrainedIndex`）
  - 参见：[性能分析](analysis/性能分析.md#b1)
- [x] P2 优化：查询定义和复杂枚举值时，如果指定了类型信息，在访问文件索引之前，预先对文件进行过滤
  - 参见：[性能分析](analysis/性能分析.md#b1)
...
- [ ] P3 优化：引入 `ParadoxMergedIndexThreadContext`，以在构建合并索引时，避免不必要的对常规缓存的的访问（configContext/configs/definitionInfo/等等） 
  - 参见：[性能分析](analysis/性能分析.md#a7)
  - 参见：[架构决策](架构决策.md#merged-index-thread-context)
- [ ] P3 优化：引入 `ParadoxMergedIndexThreadContext`，以在构建合并索引时，避免不必要的计算（memberPath/rootKeys）
  - 参见：[性能分析](analysis/性能分析.md#a7)
  - 参见：[架构决策](架构决策.md#merged-index-thread-context)
- [ ] P2 完善脚本文件的 lexer：更好的对可选空白和高级插值语法（如 `a_$p$_[[p]b]`）的支持
  - 参见：[架构决策](架构决策.md#opt-script-lexer)
- [ ] P2 尝试支持对所谓“并集作用域”或“带继承关系的作用域”的匹配与合并
