...
- 分析数据（语言区域、文件路径等）应使用文件索引（`FileBasedIndex`），因为它们只需要保存到文件级别。
- 直接基于分析数据以及 PSI 的类型与结构的数据（如封装变量、本地化）应使用存根索引（`StubIndex`），因为它们不需要依赖动态数据（如规则数据）。
- 需要先解析 PSI 引用，或者需要依赖规则数据的数据，应考虑使用文件索引（`FileBasedIndex`），同一 PSI 可能对应多条数据，并且文本范围也可能不同。
- 需要先解析 PSI 引用为模拟 PSI，或者需要依赖成员规则的数据，应考虑统一使用合并索引（`ParadoxMergedIndex`），降低重复解析（PSI 引用和规则）带来的额外耗时。
- 对于文件索引，应在必要时压缩写入的数据（如使用 `readOrReadFrom`），但不需要过度考虑优化数据结构（如使用 `FastList`）。
- 对于存在单个字符串查询参数的目标，采用1个对应的索引即可，对于存在2个字符串查询参数的目标，如果都可选，应采用3个对应的索引，以提高命中率。
