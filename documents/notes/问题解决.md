...
> 此文档用于记录开发途中发现的，需要进一步研究的各类问题，包括现象、分析、解决方案等。这些问题通常不会被视为明确的BUG。

## 构建

### 依赖处理

- 如果插件依赖的第三方库，同时又依赖 Kotlin 标准库（或者其他 IntelliJ Platform 已经包含的库，例如 Kotlin 协程库），通常需要显式排除，否则可能出现意外问题。
- 如果插件依赖的第三方库，同时又依赖注解库（如 `org.jspecify:jspecify`），通常可以显式排除，以减少插件包的大小。
...
使用尾随上下文（trailing context）的规则，如 `A / B`，相比一般规则拥有更高的优先级。
...
- 在项目启动后的规则数据的预加载全部完成后，重新解析已打开的文件（`ChronicleAnalysisManager.reparseFiles`） - 不需要与上述所有可行的方案搭配使用
...
withContext(Dispatchers.EDT) {
    runWithModalProgressBlocking(project, ChronicleBundle.message("configGroup.refresh.builtin.progressTitle")) {
...
- 通过添加系统属性来按需忽略测试（`-Dchronicle.test.include.all=true`）
...
- 如果在测试代码补全时，只有唯一一个候选项，调用 `myFixture.complete` 后会直接插入这个候选项，并且修改 PSI。
- 这可能会导致意外报错：PSI and index do not match.
- 错误堆栈显示，文件内容是插入这个候选项之后的，在修改 PSI 时未通过断言。
- 参见：`icu.windea.pls.test.issues.Issue374Test.testCompletion_DynamicValue`