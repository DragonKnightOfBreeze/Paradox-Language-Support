...
### （已解决）对于对应多个 PSI 引用的脚本表达式（如 `religion:catholic`），光标位置的用法高亮可能不正确
...
### （可能已解决）表达式 PSI 引用的缓存不一致的问题 & `Issue142Test.testVarInEffect` 可能无法通过
...
### （应当已解决）java.lang.Throwable: Write-unsafe context! Model changes are allowed from write-safe contexts only.

堆栈：[stack_trace_11](assets/stack_trace_11.txt)

自插件版本2.1.8开始，由于解析 rootInfo 时不再依赖VFS，应当不会再复现了。

### （可能已解决）表达式 PSI 引用的缓存不一致的问题 & `Issue284Test.testScopeLinkInEffect` 可能无法通过
...
### （应当已解决）在事件脚本文件中，变更文件内容后，其中特定部分的作用域推断会发生误报
...
推断：
- 应当是以下位置的异常，导致使用默认值 true，最终导致错误地随机匹配了一个模板表达式，最终导致匹配失败。
