...
### P1 （已解决）索引时报错 `Indexing process should not rely on non-indexed file data.`
...
### P2 （不再复现）`ParadoxExpressionManager.getExpressionReferences` 中使用的缓存可能不一致的问题
...
### P3 （不再复现）涉及内联脚本与适用语言注入的参数时，IDE可能卡死的问题
...
### P4 （应当已解决）进行全局代码检查时，偶发误判的 *无法解析的值表达式* 报错
...
### P5 （应当已解决）索引时的偶发报错，涉及解析定义的子类型和内联脚本

堆栈：[stack_trace_4](assets/stack_trace_4.txt)

### P6 （已解决）来自的 `ParadoxTextBasedTargetSearcher` 和 fastutil 的索引越界异常
...
### P7 （已解决）`Indexing process should not rely on non-indexed file data.` 对于子定义
...
### P9 （已解决）对于对应多个 PSI 引用的脚本表达式（如 `religion:catholic`），光标位置的用法高亮可能不正确
...
### P11 （可能已解决）表达式 PSI 引用的缓存不一致的问题 & `Issue142Test.testVarInEffect` 可能无法通过
...
### P12 （应当已解决）java.lang.Throwable: Write-unsafe context! Model changes are allowed from write-safe contexts only.

堆栈：[stack_trace_11](assets/stack_trace_11.txt)

自插件版本2.1.8开始，由于解析 rootInfo 时不再依赖VFS，应当不会再复现了。

### P13 （可能已解决）表达式 PSI 引用的缓存不一致的问题 & `Issue284Test.testScopeLinkInEffect` 可能无法通过
...
### P14 （已解决）在事件脚本文件中，变更文件内容后，其中特定部分的作用域推断会发生误报
