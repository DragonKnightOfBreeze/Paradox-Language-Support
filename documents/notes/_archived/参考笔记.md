...
- 需要提供方案验证结果的等效性。
- 避免无限循环调用。
- 不要更改直接调用 `CachedValuesManager.getCachedValue(...)` 的那个顶级方法（静态方法）的方法声明，IDE 内部会进行检查。
...
- `com.intellij.ui.dsl.builder.Cell.onApply` - 调用这个方法时，对应的属性必定已经被修改（但是，如果修改了多个属性并且存在重复的对应的回调，似乎仍然会重复执行回调）
