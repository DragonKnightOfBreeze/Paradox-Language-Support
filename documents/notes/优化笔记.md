...
结论：
- 规则对象仍然占用了一些内存
- 优化 `PsiCodeInjectors` - 仅缓存必要的字段 ✔ + 考虑适用 `String.intern()`
- 优化 `ParadoxElementPath`（以及其他路径模型） - 考虑适用 `String.intern()` + 考虑适用有上限的缓存
...
| gfx                                                          | 1571      | 37704          |
| GetName                                                      | 1534      | 36816          |
| federation                                                   | 1525      | 36600          |
| : event, triggered, hidden, country                          | 1451      | 34824          |
| 1                                                            | 1438      | 34512          |
| set_country_flag                                             | 1430      | 34320          |
| (property) id =  (property) id =  (property) days = int_value_field (property) months = int_value_field (property) years = int_value_field (property) random = int_value_field (property) scopes = {...} | 1409      | 33816          |
| option/hidden_effect                                         | 1380      | 33120          |
| option/trigger                                               | 1343      | 32232          |
| country_event/is_triggered_only                              | 1281      | 30744          |
| desc/trigger                                                 | 1234      | 29616          |
| (property) trigger = single_alias_right[trigger_clause] (property) exclusive_trigger = single_alias_right[trigger_clause] (property) text = localisation (property) show_sound = | 1226      | 29424          |
| common                                                       | 1220      | 29280          |
| country_event/option/hidden_effect                           | 1164      | 27936          |
| effect                                                       | 1156      | 27744          |
| true                                                         | 1151      | 27624          |
| add_resource                                                 | 1143      | 27432          |
| desc/text                                                    | 1137      | 27288          |
| option/custom_tooltip                                        | 1129      | 27096          |
| AND                                                          | 1105      | 26520          |
| country_event/option/trigger                                 | 1103      | 26472          |
| country_event/immediate                                      | 1054      | 25296          |
| days                                                         | 1050      | 25200          |
| leader                                                       | 1022      | 24528          |
| particles                                                    | 1005      | 24120          |
| factor                                                       | 1005      | 24120          |

- 原来缓存键也需要 `String.intern()`……
- `immediate` - 高频出现的脚本属性的 key/value 也需要考虑 `String.intern()`
- `GetName` - 高频出现的其他标识符（这里是本地化命令）也需要考虑 `String.intern()`
- `: event_target` - 快速文档和内嵌提示中对类型信息的提示，相关的字符串也需要 `String.intern()`
- `(property) alias_name[trigger] = alias_match_left[trigger]` - 需要定位并优化
  - 来自 `ParadoxScriptBlockImpl` 中的 userData
  - 最终来自 `icu.windea.pls.lang.util.ParadoxExpressionManager.getChildOccurrenceMap`，需要优化
